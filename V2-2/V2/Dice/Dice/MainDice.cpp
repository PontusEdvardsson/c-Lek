...
int main()
{
	...
	*/
	...
	delete[] d5;
	*/
	Dice* *d7 = new Dice*[3]; // dubbelpekare första * för att allokera dynamiskt. ** för att dynamiskt allokera pekare
	Dice **d8 = nullptr;
	d8 = new Dice*[3];
	for (int i = 0; i < 3; i++)
	{
	...
	d8[0] = new Dice(7);
	d8[1] = new Dice(19); // både arrayen med pekarna och dice objekten i heapen
	for (int i = 0; i < 2; i++)
	{
		d8[i]->toss();
		cout << d8[i]->getValue() << endl;
	}
	//behöver en forloop som deletar det pointersen pekar på, sedan en för att deletar pointersen.
	for (int i = 0; i < 2; i++)
	{
		delete d8[i];   //deletar objekten som pekas på
	}
	delete[] d8; //deletar arrayen, nu är allt i heapen deletat
